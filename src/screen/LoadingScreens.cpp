...
bool LoadingLinesScreen::hasNextScreen() {
    return next;
}

void LoadingShrimpBoardScreen::begin() {
    Screen::begin();
    previousMillis = millis();
    next = false;
    step = 0;
...
void LoadingShrimpBoardScreen::loop() {
    unsigned long currentMillis = millis();
    int type = getSettings().getLoadingScreenType();
...
bool LoadingShrimpBoardScreen::hasNextScreen() {
    return next;
}