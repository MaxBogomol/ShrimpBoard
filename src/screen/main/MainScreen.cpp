...
bool MainScreen::hasNextScreen() {
    return next;
}

void MainScreen::drawStats(int x, int y) {
    getDisplay().setTextPos(x, y + getShrimpBoard().isUSBMode() ? 4 : 20);
    getDisplay().drawText(">");
...
void MainScreen::drawKeyboard(int x, int y) {
    ...
    bool numLock = ((getShrimpBoard().isUseUSB() && getSettings().isNumLockUSB()) || (getShrimpBoard().isUseBLE() && getSettings().isNumLockBLE()));
    bool capsLock = ((getShrimpBoard().isUseUSB() && getSettings().isCapsLockUSB()) || (getShrimpBoard().isUseBLE() && getSettings().isCapsLockBLE()));
    bool scrollLock = ((getShrimpBoard().isUseUSB() && getSettings().isScrollLockUSB()) || (getShrimpBoard().isUseBLE() && getSettings().isScrollLockBLE()));
    ...
    if (getKeyboard().isTwoLinkedButtonPressed(5, 4, 5, 5)) getDisplay().drawRectangle(x + 16, y + 21, 20, 2, getDisplay().white());
