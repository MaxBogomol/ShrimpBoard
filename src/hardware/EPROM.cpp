...
Settings& EPROM::getSettingsSaved() {
    return *this->settingsSaved;
}

bool EPROM::readBool(int address) {
    bool value;
    eprom->get(address, value);
    return value;
}

int EPROM::readInt(int address) {
    int value;
    eprom->get(address, value);
    return value;
}
...
float EPROM::readFloat() {
    float value = readFloat(index);
    index += 4;
    return value;
}

int EPROM::readIntLimit(int minValue, int maxValue, int standardValue) {
    int value = readInt();
    if (value < minValue || value > maxValue) value = standardValue;
    return value;
}

float EPROM::readFloatLimit(float minValue, float maxValue, float standardValue) {
    float value = readFloat();
    if (value < minValue || value > maxValue) value = standardValue;
    return value;
}

void EPROM::writeBool(int address, bool value) {
    eprom->put(address, value);
}

void EPROM::writeInt(int address, int value) {
    eprom->put(address, value);
}
...
void EPROM::writeBool(bool value) {
    writeBool(index, value);
    index += 1;
}

void EPROM::writeInt(int value) {
    writeInt(index, value);
    index += 2;
}

void EPROM::writeFloat(float value) {
    writeFloat(index, value);
    index += 4;
}

void EPROM::writeBoolIf(bool write, bool value) {
    if (write) writeBool(index, value);
    index += 1;
}

void EPROM::writeIntIf(bool write, int value) {
    if (write) writeInt(index, value);
    index += 2;
}

void EPROM::writeFloatIf(bool write, float value) {
